 ...
 * @date 2021/9/1
 */
public class ProjectSubFragment extends JzFragment implements SwipeRefreshLayout.OnRefreshListener {

    public final static String TAB_INDEX = "tab_index";
    public final static String TREE_ID = "treeId";
    public final static String IS_NEW = "isNews";
    ...
    public static ProjectSubFragment newInstance(int tab, int mTreeId, boolean isNew) {
        final ProjectSubFragment fragment = new ProjectSubFragment();
    ...
    private void requestProjects() {
        ...
        if (mIsNew) {
            url = NetManager.getInstance().getUrl(NetParams.getPageUrl(ConstUrl.PROJECT_LIST, mPage));
        } else {
            url = NetManager.getInstance().getUrl(NetParams.getPageUrl(ConstUrl.PROJECT_LIST_BY_TREE, mPage));
            params.put("cid", mTreeId);
        }
        RxNetClient.builder()
                .url(url)
                .params(params)
